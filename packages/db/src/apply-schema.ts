...
ALTER TABLE contacts ADD COLUMN IF NOT EXISTS deleted_reason text;
CREATE INDEX IF NOT EXISTS contacts_deleted_at_idx ON contacts (tenant_id, deleted_at) WHERE deleted_at IS NOT NULL;
CREATE INDEX IF NOT EXISTS contacts_active_idx ON contacts (tenant_id) WHERE deleted_at IS NULL AND archived_at IS NULL;
