...
async function ensureStageBelongsToTenant(
  tx: TenantContextDb,
  tenantId: string,
  stageId: string
): Promise<void> {
...
export async function getPipeline(): Promise<StageWithOpportunities[]> {
  return withAuthContext(async (auth, tx) => {
    ...
    const oppsWithContact = await tx
      .select({
        id: opportunities.id,
        title: opportunities.title,
        caseType: opportunities.caseType,
...
export async function updateOpportunityStage(
  ...
  await withAuthContext(async (auth, tx) => {
    ...
    await ensureStageBelongsToTenant(tx, auth.tenantId, stageId);
...
export async function getOpportunityById(id: string): Promise<OpportunityDetail | null> {
  return withAuthContext(async (auth, tx) => {
    ...
    const stageRows = await tx
      ...
      .where(eq(opportunityStages.tenantId, auth.tenantId))
      .orderBy(asc(opportunityStages.sortOrder));
    const stages: OpportunityStageInfo[] = stageRows.map((r) => ({
      id: r.id,
      name: r.name,
      sortOrder: r.sortOrder,
      probability: r.probability ?? null,
...
export async function createOpportunity(data: {
  ...
  const newId = await withAuthContext(async (auth, tx) => {
    if (!hasPermission(auth.roleName, "opportunities:write")) throw new Error("Forbidden");
    await ensureStageBelongsToTenant(tx, auth.tenantId, data.stageId);
    if (data.contactId) {
