...
/**
 * Dev-only seed. V produkci chráněno permission guardem + předpokládá se, že
 * běží jen v development tenantech. Wrapnuté v `withAuthContext`, aby po swapu
 * runtime DB role na `aidvisora_app` insert operace proběhly pod správnými GUC
...
export async function seedDemoData(): Promise<{ ok: boolean; message: string }> {
  return withAuthContext(async (auth, tx) => {
    ...
    const existingHouseholds = await tx
    ...
    if (stageIds.length === 0) {
      ...
      for (let i = 0; i < defaultStages.length; i++) {
        await tx.insert(opportunityStages).values({
          tenantId: auth.tenantId,
    ...
    for (const n of demoNotes) {
      ...
      await tx.insert(meetingNotes).values({
        tenantId: auth.tenantId,
        contactId,
