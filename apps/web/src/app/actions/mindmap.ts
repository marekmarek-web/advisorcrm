...
export async function getMindmapByMapId(mapId: string): Promise<MindmapState | null> {
  return withAuthContext(async (auth, tx) => {
    ...
    const nodesRows = await tx
      .select()
      .from(mindmapNodes)
      .where(eq(mindmapNodes.mapId, map.id));
    const edgesRows = await tx
      .select()
      .from(mindmapEdges)
...
export async function deleteStandaloneMap(mapId: string): Promise<{ ok: boolean }> {
  return withAuthContext(async (auth, tx) => {
    ...
    await tx.delete(mindmapMaps).where(eq(mindmapMaps.id, mapId));
    return { ok: true };
  });
}
...
export async function duplicateStandaloneMap(mapId: string): Promise<{ mapId: string }> {
  ...
  return withAuthContext(async (auth, tx) => {
    ...
    if (edgeValues.length > 0) {
      await tx.insert(mindmapEdges).values(edgeValues);
    }
  ...
  return withAuthContext(async (auth, tx) => {
    ...
    if (entityType === "standalone") {
      ...
      await tx
        ...
        .where(eq(mindmapMaps.id, mapId));
      await tx.delete(mindmapNodes).where(eq(mindmapNodes.mapId, mapId));
      await tx.delete(mindmapEdges).where(eq(mindmapEdges.mapId, mapId));
    } else {
      const [existing] = await tx
        .select({ id: mindmapMaps.id })
        .from(mindmapMaps)
