...
export function LegalBlocks(props: { blocks: LegalBlock[]; sectionIdPrefix?: string }) {
  const { blocks, sectionIdPrefix = "" } = props;
  ...
  while (i < blocks.length) {
    ...
    if (b.type === "h1") {
      ...
      nodes.push(
        <h2
          key={`h-${key++}`}
          id={`${sectionIdPrefix}sec-${sectionIndex}`}
          className="scroll-mt-24 text-lg font-semibold text-gray-900 dark:text-white mt-10"
        >
          {b.text}
        </h2>
      );
  ...
  return <div className="space-y-1 [&_h2:first-of-type]:mt-0">{nodes}</div>;
}
