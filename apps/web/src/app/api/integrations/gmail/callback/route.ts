...
import { upsertGmailTokens, logGmailIntegration, logGmailIntegrationError } from "@/lib/integrations/google-gmail-integration-service";
...
function redirectToSetup(origin: string, success?: boolean, error?: string): NextResponse {
  ...
  target.search = "tab=integrace" + (success ? "&gmail=connected" : error ? "&gmail_error=" + encodeURIComponent(error) : "");
...
export async function GET(request: Request) {
  ...
  if (errorParam) {
    logGmailIntegration("Callback: denied", { error: errorParam });
    return redirectToSetup(origin, false, errorParam === "access_denied" ? "access_denied" : "auth_failed");
  }

  if (!code || !stateParam) {
    logGmailIntegrationError("Callback: missing code or state");
  ...
  try {
    ...
    logGmailIntegrationError("Callback: invalid state", e);
  ...
  if (!csrf || csrf !== state.nonce) {
    logGmailIntegrationError("Callback: CSRF mismatch");
  ...
  if (!user || user.id !== state.userId) {
    logGmailIntegrationError("Callback: user mismatch");
  ...
  try {
    ...
    logGmailIntegrationError("Callback: token exchange failed", e);
    return redirectToSetup(origin, false, "token_exchange_failed");
  }

  if (!tokens.refresh_token) {
    logGmailIntegrationError("Callback: no refresh_token");
  ...
  try {
    await upsertGmailTokens(state.userId, state.tenantId, {
    ...
    logGmailIntegrationError("Callback: DB save failed", e);
    return redirectToSetup(origin, false, "db_error");
  }

  logGmailIntegration("Callback: success", { userId: state.userId.slice(0, 8) });
