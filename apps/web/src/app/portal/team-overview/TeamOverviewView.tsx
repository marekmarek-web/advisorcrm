...
import {
  ...
  memberMatchesPeopleSegment,
  type PeopleSegmentFilter,
} from "@/lib/team-overview-page-model";
...
export function TeamOverviewView({
  ...
  const visibleMembers = sortedMembers.filter((m) => {
    ...
    if (!mm) return true;
    if (
      !memberMatchesPeopleSegment(m, peopleSegment, {
        metricsByUser,
        newcomerSet,
  ...
  const briefing = pageModel.briefing;
      ...
      <div className="mx-auto max-w-7xl px-3 sm:px-6 lg:px-8 py-6 md:py-8">
        ...
        >
          <div className="mb-6 max-w-3xl">
            ...
            <p className="mt-3 border-l-2 border-indigo-200 pl-3 text-xs leading-relaxed text-[color:var(--wp-text-tertiary)]">{briefing.valueFramingLine}</p>
          </div>
          {kpis ? (
          ...
          >
            ...
            <div className="mt-4 rounded-xl border border-[color:var(--wp-surface-card-border)] bg-[color:var(--wp-surface-muted)]/35 px-4 py-3 text-xs text-[color:var(--wp-text-secondary)] space-y-1">
              <p>
                ...
                {pageModel.poolSplit.counts.not_set} {pageModel.poolSplit.counts.not_set === 1 ? "osoba" : "lidí"}
              </p>
              <p>
        ...
        <div className="mb-4 flex flex-wrap items-center gap-2">
          ...
          <CustomDropdown
            value={peopleSegment}
            onChange={(id) => setPeopleSegment(id as PeopleSegmentFilter)}
            options={[
