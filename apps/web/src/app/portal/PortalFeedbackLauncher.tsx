...
export function PortalFeedbackLauncher({ variant = "desktop" }: { variant?: "desktop" | "mobile" | "mobileHeader" }) {
  ...
  const isMobileVariant = variant === "mobile" || variant === "mobileHeader";
  ...
  const triggerClass =
    variant === "mobileHeader"
      ? "grid min-h-[48px] min-w-[48px] place-items-center rounded-[20px] border border-white/70 bg-white/55 text-[color:var(--wp-text)] shadow-[0_12px_26px_rgba(15,23,42,0.07)] ring-1 ring-[color:var(--wp-surface-card-border)]/40 backdrop-blur-2xl transition-transform active:scale-95"
      : `fixed ${positionClass} flex min-h-[48px] min-w-[48px] items-center justify-center rounded-full border-2 border-[color:var(--wp-surface-card-border)] bg-[color:var(--wp-surface-muted)] text-[color:var(--wp-text)] shadow-md transition-all hover:bg-[color:var(--wp-surface-muted)] hover:border-slate-400 active:scale-95 dark:border-slate-400 dark:bg-slate-500 dark:text-white dark:hover:bg-slate-400 dark:hover:border-[color:var(--wp-surface-card-border)]`;
  ...
  const desktopModal =
    open && !isMobileVariant && mounted ? (
      createPortal(
        ...
        >
          <div className="bg-[color:var(--wp-surface-card)] rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-md max-h-[90vh] min-h-[min(85dvh,640px)] flex flex-col">
            ...
            <div className="p-4 space-y-4 overflow-y-auto flex-1 min-h-0">{formFields}</div>
            <div className="p-4 border-t border-[color:var(--wp-surface-card-border)] flex gap-2 shrink-0">{footerButtons}</div>
          </div>
        </div>,
        document.body,
      )
  ...
  return (
    <>
      ...
      >
        <Bug size={22} className="text-[color:var(--wp-text)] dark:text-white" strokeWidth={2.25} />
      </button>
